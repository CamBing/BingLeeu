...
\item when running dccfit() we get the following error: [Error in -x@fit$log.likelihoods : invalid argument to unary operator]
Cannot find a solution online that is relevant to what we are doing.
...
Pct_Valid <- 0.70  # This can change of course. 70% valid data over period at least
StartDate <- ymd(20050101)
EndDate <- ymd(20171031)
# Previously you were not using spread_by explicitly:
mergeddataset <- 
  mergeddataset %>% filter(date >= StartDate & date <= EndDate)
...
Tickers_Active_At_Last <- 
  ...
  group_by(Ticker) %>% 
  mutate(N_Valid = ifelse(DlogReturn == 0, 0, 1) ) %>% summarise(S = sum(N_Valid)) %>% 
